...
const TestForm = () => {
  const { addTest, editTest, modal, closeModal, populatedList } = useStore();
  ...
  const categoryOptions = populatedList.map((cat) => ({
    value: cat._id,
  ...
  const handleSubmit = () => {
    if (modal.view === "addTestForm") {
      ...
      editTest(categoryId, selectedCategoryId, testId, name);
    }
  };
