  ...
  return (
    <div className="space-y-4 md:space-y-6">
      {categoryList.map((item) => {
        ...
        const liveCount = item.testList.filter((test) => test.schemaId).length;
        const setupCount = item.testList.filter((test) => !test.schemaId).length;
        return (
          <div
            key={item._id}
          ...
          >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 md:gap-4 mt-1 md:mt-2 flex-wrap">
                    ...
                    <span className="inline-flex items-center gap-1 bg-gray-100 text-gray-700 px-2 md:px-3 py-1 rounded-full text-xs md:text-sm">
                      ...
                      {item.testList.length} {item.testList.length === 1 ? "Test" : "Tests"}
                    </span>
                    {/* Live Count Badge */}
                    {liveCount > 0 && (
            ...
            {/* Tests Section */}
            <div>
              <TestList categoryId={item._id} list={item.testList} /> {/* Fixed: item._id instead of item.categoryId */}
            </div>
          </div>
        );
      })}
